...
unsafe extern "C" fn usrsctp_write_callback(
    ...
    if let Some(socket) = socket {
        let buffer = slice_from_raw_parts_mut(buffer as *mut u8, length).as_mut().unwrap();
...
unsafe extern "C" fn usrsctp_read_callback(
    ...
    if let Some(socket) = socket {
        if data.is_null() {
            ...
            let buffer = slice_from_raw_parts_mut(data as *mut u8, datalen).as_mut().unwrap();
